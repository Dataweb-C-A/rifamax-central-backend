class X100::RafflesController < ApplicationController
  ...
  def index
    ...
    render json: @x100_raffles.last, status: :ok
  end
  ...
  def create
    ...
    if @current_user.role == "Taquilla"
      @x100_raffle.shared_user_id = @current_user.id
    end
  ...
  def destroy
    if @x100_raffle_taquilla.nil?
      ...
      render json: { message: "Raffle deleted", raffle: @x100_raffle_taquilla }, status: :ok, location: @x100_raffle_taquilla
    end
  end
  ...
  def set_x100_raffle
    @x100_raffle_taquilla

    if @current_user.role == 'Admin'
      @x100_raffle_taquilla = X100::Raffle.find(params[:id])
    else
      @x100_raffle_taquilla = X100::Raffle.where(id: params[:id], shared_user_id: @current_user.id).last
    end
  end
